...
export default function HospitalHomePage({ onNavigate, isLoggedIn, userRole, onLogout, onNavigateToResource, onNavigateToAdminDashboard, onNavigateToProgram, onNavigateToResearch }) {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch('/api/homepage-gallery').then(r => r.json()).catch(() => ({ success: false })),
    ]).then(([fac, ach, ev, nw, svc, ct, sl, gal]) => {
      ...
      if (gal.success) setGalleryImages(gal.images);
    }).catch(console.error);
  }, []);
  ...
  }, [galleryIndex, galleryPaused, galleryImages.length]);

  const galleryGoTo = (idx) => {
  ...
  const galleryPrev = () => galleryGoTo((galleryIndex - 1 + galleryImages.length) % galleryImages.length);
  const galleryNext = () => galleryGoTo((galleryIndex + 1) % galleryImages.length);
  ...
  const computeCropCSS = (cx, cy, cw, ch) => {
    cx = cx ?? 0; cy = cy ?? 0; cw = cw ?? 100; ch = ch ?? 100;
    if (cw >= 99.9 && ch >= 99.9) return { backgroundSize: 'cover', backgroundPosition: 'center center' };
    const bsW = (10000 / cw).toFixed(2);
    const bsH = (10000 / ch).toFixed(2);
    const bpX = (100 - cw) > 0.1 ? Math.min(100, (cx * 100) / (100 - cw)).toFixed(2) : 0;
    const bpY = (100 - ch) > 0.1 ? Math.min(100, (cy * 100) / (100 - ch)).toFixed(2) : 0;
    return { backgroundSize: `${bsW}% ${bsH}%`, backgroundPosition: `${bpX}% ${bpY}%` };
  };
  ...
  return (
    <div style={{ background: 'linear-gradient(160deg, #fdf6f0 0%, #fef0f5 40%, #f5f0fe 100%)', fontFamily: "'Georgia', 'Times New Roman', serif" }} className="text-stone-700 min-h-screen">
      ...
      <style>{`
        ...
        .gallery-img-enter { animation: galleryKenBurns 3.4s ease-out forwards; }
        .gallery-img-fade { opacity: 0; transition: opacity 0.6s ease; }
        .gallery-img-visible { opacity: 1; transition: opacity 0.6s ease; }
        .gallery-nav-btn {
          width: 44px; height: 44px; border-radius: 50%;
          background: rgba(255,248,252,0.75);
          border: 1px solid rgba(210,150,170,0.35);
          backdrop-filter: blur(8px);
          display: flex; align-items: center; justify-content: center;
          cursor: pointer; transition: all 0.25s;
          color: #c9608c;
          box-shadow: 0 4px 18px rgba(200,100,140,0.15);
        }
        .gallery-nav-btn:hover {
          background: rgba(255,235,245,0.95);
          border-color: rgba(201,96,140,0.55);
          box-shadow: 0 6px 24px rgba(200,100,140,0.28);
          transform: scale(1.08);
        }
        .gallery-dot {
          width: 7px; height: 7px; border-radius: 50%;
          cursor: pointer; transition: all 0.3s;
          border: 1.5px solid rgba(201,96,140,0.4);
        ...
        .gallery-dot-active {
          ...
          box-shadow: 0 2px 8px rgba(200,100,140,0.35);
        }
      `}</style>
      ...
      <div className={`${scrolled ? 'fixed top-0' : 'fixed top-20'} left-0 right-0 z-40 glass-nav transition-all duration-300`}>
        <div className="w-full px-4 sm:px-6 lg:px-8">
          <div className="hidden lg:flex items-center justify-between px-4 py-3">
            {(isLoggedIn
              ? ['home', 'faculty', 'about', 'programs', 'resources', 'discussion', 'research', 'achievements', 'contact']
              : ['home', 'faculty', 'about', 'achievements', 'contact']
            ).map(section => (
      ...
      {/* Hero Section — gallery lives inside here */}
      <section id="home" className="hero-glow relative min-h-screen flex flex-col items-center justify-center pt-28 pb-20 noise-overlay">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <p className="tag-rose mb-3">HITEC Institute of Medical Sciences</p>
          <h1 style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontWeight: 300, lineHeight: 1.15, color: '#3d2535' }} className="text-5xl md:text-7xl mb-5">Department of Obstetrics<br /><em style={{ fontStyle: 'italic', color: '#b06080' }}>& Gynecology</em></h1>
          <p style={{ fontFamily: "'Jost', sans-serif", fontWeight: 300, color: '#9a7080' }} className="text-xl md:text-2xl mb-4 max-w-3xl mx-auto">Excellence in Women&apos;s Healthcare</p>
          <p className="body-text text-base max-w-2xl mx-auto mb-0">Providing comprehensive obstetric and gynecological care with state-of-the-art facilities and experienced faculty</p>

          {/* Members portal CTA — only for guests */}
          {!isLoggedIn && (
            <div style={{ marginTop: '2rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 20px', borderRadius: 40, background: 'rgba(255,240,248,0.65)', border: '1px solid rgba(210,140,170,0.3)', backdropFilter: 'blur(8px)' }}>
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="#c9608c" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>
                <span style={{ color: '#9a6878', fontFamily: "'Jost', sans-serif", fontWeight: 300, fontSize: '0.82rem', letterSpacing: '0.02em' }}>Training programs, resources & research are available to members</span>
              </div>
              <button onClick={() => onNavigate('login')} className="rose-btn px-8 py-3 rounded-full font-medium transition-all" style={{ fontFamily: "'Jost', sans-serif", fontSize: '0.85rem', letterSpacing: '0.06em', textTransform: 'uppercase' }}>
                Member Login &nbsp;→
              </button>
            </div>
          )}
        ...
        {/* Gallery carousel — seamlessly embedded in hero, no card/box */}
        {galleryImages.length > 0 && (
          <div
            className="relative z-10 w-full"
            style={{ maxWidth: '520px', margin: '2.4rem auto 0', padding: '0 1.25rem' }}
          ...
          >
            ...
            <div
              ...
              style={{
                borderRadius: '22px',
                background: 'linear-gradient(160deg, #fdf6f0 0%, #fef0f5 50%, #f5f0fe 100%)',
                boxShadow: '0 8px 40px rgba(190,100,150,0.13), 0 1px 4px rgba(190,100,150,0.08)',
                border: '1px solid rgba(220,175,185,0.22)',
              }}
            >
              {galleryImages.map((img, idx) => {
                const cx = img.crop_x ?? 0, cy = img.crop_y ?? 0;
                const cw = img.crop_w ?? 100, ch = img.crop_h ?? 100;
                const hasCrop = !(cw >= 99.9 && ch >= 99.9);
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      top: 0, left: 0,
                      width: '100%',
                      opacity: idx === galleryIndex && !galleryFading ? 1 : 0,
                      transition: 'opacity 0.72s cubic-bezier(0.4,0,0.2,1)',
                      zIndex: idx === galleryIndex ? 2 : 1,
                    }}
                  >
                    {hasCrop ? (
                      // Admin-cropped: background-image fills container with chosen crop region
                      <div style={{
                        width: '100%',
                        height: 'clamp(340px, 55vh, 520px)',
                        backgroundImage: `url(${img.image_url})`,
                        backgroundRepeat: 'no-repeat',
                        ...computeCropCSS(cx, cy, cw, ch),
                        position: 'relative',
                      }}>
                        {img.caption && (
                          <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '14px 22px', background: 'linear-gradient(to top, rgba(42,16,32,0.35), transparent)', pointerEvents: 'none', borderRadius: '0 0 22px 22px' }}>
                            <p style={{ color: 'rgba(255,238,248,0.92)', fontFamily: "'Jost', sans-serif", fontWeight: 300, fontSize: '0.88rem', letterSpacing: '0.03em', textShadow: '0 1px 5px rgba(0,0,0,0.25)' }}>{img.caption}</p>
                          </div>
                        )}
                      </div>
                    ) : (
                      // No crop: natural image dimensions, zero side gaps
              ...
              {galleryImages.length > 1 && (
                <>
                  <button onClick={galleryPrev} className="gallery-nav-btn" style={{ position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)', zIndex: 20 }} aria-label="Previous image">
                  ...
                  <button onClick={galleryNext} className="gallery-nav-btn" style={{ position: 'absolute', right: 14, top: '50%', transform: 'translateY(-50%)', zIndex: 20 }} aria-label="Next image">
                    <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.3" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 18 15 12 9 6"/></svg>
                  </button>
                </>
              )}
            </div>
            ...
            {galleryImages.length > 1 && (
              <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginTop: 13 }}>
                {galleryImages.map((_, idx) => (
                  <button key={idx} onClick={() => galleryGoTo(idx)} className={`gallery-dot ${idx === galleryIndex ? 'gallery-dot-active' : ''}`} aria-label={`Go to image ${idx + 1}`} />
                ))}
              </div>
            )}
          </div>
        ...
        {/* Scroll mouse */}
        <div className="absolute bottom-2 left-1/2 animate-bounce" style={{ transform: 'translateX(-50%)' }}>
      ...
      {/* Programs Section — members only */}
      {isLoggedIn && <section id="programs" className="py-20 section-alt">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      {/* Resources Section — members only */}
      {isLoggedIn && <section id="resources" className="py-20 section-main">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
      ...
      {/* Discussion Forum Section — members only */}
      {isLoggedIn && <section id="discussion" className="py-20 section-alt">
      ...
      {/* Research Section — members only */}
      {isLoggedIn && <section id="research" className="py-20 section-main">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-8">
            ...
            <div>
              ...
              <div className="space-y-2">
                {(isLoggedIn ? ['home', 'faculty', 'about', 'programs', 'research'] : ['home', 'faculty', 'about']).map(s => (
