    ...
    <div style={{ background: 'linear-gradient(160deg, #fdf6f0 0%, #fef0f5 40%, #f5f0fe 100%)', fontFamily: "'Georgia', 'Times New Roman', serif" }} className="text-stone-700 min-h-screen">
      ...
      {galleryImages.length > 0 && (
        <section className="py-20 section-alt relative overflow-hidden">
          <div className="absolute inset-0" style={{ background: 'radial-gradient(ellipse at 60% 50%, rgba(230,160,185,0.10) 0%, transparent 65%), radial-gradient(ellipse at 20% 80%, rgba(185,155,220,0.08) 0%, transparent 55%)' }} />
          <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div className="text-center mb-10">
              <span className="tag-rose">Gallery</span>
              <h2 style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontWeight: 300, color: '#3d2535' }} className="text-4xl md:text-5xl mt-2">Department Highlights</h2>
            </div>
            <div
            ...
            >
              {/* Card frame */}
              <div className="relative glass-card rounded-3xl overflow-hidden" style={{ boxShadow: '0 20px 70px rgba(190,110,150,0.18), 0 1px 0 rgba(255,255,255,0.8) inset' }}>
                {/* Image area */}
                <div className="relative w-full overflow-hidden" style={{ aspectRatio: '16/7', background: 'linear-gradient(135deg,#fdf0f5,#f5f0fe)' }}>
                  {galleryImages.map((img, idx) => (
                    <div
                      key={img.id}
                      className={`absolute inset-0 ${galleryFading || idx !== galleryIndex ? 'gallery-img-fade' : 'gallery-img-visible'}`}
                      style={{ zIndex: idx === galleryIndex ? 2 : 1 }}
                    >
                      <img
                        src={img.image_url}
                        alt={img.caption || `Gallery image ${idx + 1}`}
                        className={idx === galleryIndex ? 'gallery-img-enter' : ''}
                        style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }}
                      />
                      {/* Soft gradient overlay at bottom */}
                      <div className="absolute inset-0" style={{ background: 'linear-gradient(to top, rgba(45,20,35,0.38) 0%, transparent 50%)' }} />
                    </div>
                  ))}
                  ...
                  {galleryImages[galleryIndex]?.caption && (
                    <div className="absolute bottom-0 left-0 right-0 px-8 pb-6 z-10">
                      <p style={{ color: 'rgba(255,245,250,0.92)', fontFamily: "'Jost', sans-serif", fontWeight: 300, fontSize: '0.95rem', letterSpacing: '0.03em', textShadow: '0 1px 6px rgba(0,0,0,0.3)' }}>
                        {galleryImages[galleryIndex].caption}
                      </p>
                    </div>
                  )}
                  ...
                  {galleryImages.length > 1 && (
                    <>
                      <button
                        onClick={galleryPrev}
                        className="gallery-nav-btn absolute left-4 top-1/2 z-10"
                      ...
                      <button
                        onClick={galleryNext}
                        className="gallery-nav-btn absolute right-4 top-1/2 z-10"
                        style={{ transform: 'translateY(-50%)' }}
                ...
                {/* Dot indicators */}
                {galleryImages.length > 1 && (
                  <div className="flex items-center justify-center gap-2 py-5" style={{ background: 'rgba(255,250,252,0.65)' }}>
                    {galleryImages.map((_, idx) => (
                      <button
                        key={idx}
                        onClick={() => galleryGoTo(idx)}
                        className={`gallery-dot ${idx === galleryIndex ? 'gallery-dot-active' : ''}`}
              ...
              {/* Decorative glow */}
              <div className="absolute -inset-px rounded-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none" style={{ background: 'linear-gradient(135deg, rgba(232,160,176,0.15), rgba(185,155,220,0.12), rgba(232,160,176,0.15))', zIndex: -1 }} />
            </div>
          </div>
        </section>
      )}
