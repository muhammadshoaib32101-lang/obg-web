...
export default function AdminDashboard({ onLogout, userToken, onNavigateToSignup, onNavigateToHome }) {
  ...
  const [cropModalBox, setCropModalBox] = useState({ x: 0, y: 0, w: 100, h: 100 });
  const [cropSaving, setCropSaving] = useState(false);
  const cropContainerRef = useRef(null);
  const cropDragRef = useRef({ active: false });
  ...
  useEffect(() => {
    ...
    else if (activeTab==='events-news') { fetchEvents(); fetchNews(); fetchGallery(); }
  ...
  // ── gallery handlers ─────────────────────────────────────────────────────
  const fetchGallery = async () => { setGalleryLoading(true); try{const r=await fetch('/api/homepage-gallery');const d=await r.json();if(d.success)setGallery(d.images);}catch(e){}finally{setGalleryLoading(false);} };

  const openCropModal = (img) => {
    setCropModalImage(img);
    setCropModalBox({ x: img.crop_x ?? 0, y: img.crop_y ?? 0, w: img.crop_w ?? 100, h: img.crop_h ?? 100 });
    setShowCropModal(true);
  };

  const startCropDrag = (e, type) => {
    e.preventDefault(); e.stopPropagation();
    cropDragRef.current = { active: true, type, startMouse: { x: e.clientX, y: e.clientY }, startBox: { ...cropModalBox } };
  };

  const onCropMouseMove = useCallback((e) => {
    const d = cropDragRef.current;
    if (!d.active || !cropContainerRef.current) return;
    ...
    switch (d.type) {
      case 'move': nx = clamp(0, 100 - b.w, b.x + dx); ny = clamp(0, 100 - b.h, b.y + dy); break;
      case 'nw': { const ex = b.x + b.w, ey = b.y + b.h; nx = clamp(0, ex - MIN, b.x + dx); ny = clamp(0, ey - MIN, b.y + dy); nw = ex - nx; nh = ey - ny; break; }
      case 'ne': { const ey = b.y + b.h; ny = clamp(0, ey - MIN, b.y + dy); nh = ey - ny; nw = clamp(MIN, 100 - b.x, b.w + dx); break; }
      case 'sw': { const ex = b.x + b.w; nx = clamp(0, ex - MIN, b.x + dx); nw = ex - nx; nh = clamp(MIN, 100 - b.y, b.h + dy); break; }
      case 'se': nw = clamp(MIN, 100 - b.x, b.w + dx); nh = clamp(MIN, 100 - b.y, b.h + dy); break;
      case 'n': { const ey = b.y + b.h; ny = clamp(0, ey - MIN, b.y + dy); nh = ey - ny; break; }
      case 's': nh = clamp(MIN, 100 - b.y, b.h + dy); break;
      case 'w': { const ex = b.x + b.w; nx = clamp(0, ex - MIN, b.x + dx); nw = ex - nx; break; }
      case 'e': nw = clamp(MIN, 100 - b.x, b.w + dx); break;
    }
    setCropModalBox({ x: nx, y: ny, w: nw, h: nh });
  }, []);

  const onCropMouseUp = useCallback(() => { cropDragRef.current.active = false; }, []);

  const handleApplyCrop = async () => {
    setCropSaving(true);
    try {
      const r = await fetch('/api/homepage-gallery', { method: 'PUT', headers: jsonH(), body: JSON.stringify({ id: cropModalImage.id, crop_x: cropModalBox.x, crop_y: cropModalBox.y, crop_w: cropModalBox.w, crop_h: cropModalBox.h }) });
      const d = await r.json();
      if (d.success) { showToast('Crop saved!'); fetchGallery(); setShowCropModal(false); }
  ...
  useEffect(() => {
    if (showCropModal) {
      ...
      return () => { window.removeEventListener('mousemove', onCropMouseMove); window.removeEventListener('mouseup', onCropMouseUp); };
    }
  }, [showCropModal, onCropMouseMove, onCropMouseUp]);
  const handleGalleryFileSelect = async e => {
    const file = e.target.files?.[0];
    if (!file) return;
    if (file.type !== 'image/jpeg') { showToast('Only JPEG images are allowed', 'error'); if(galleryFileRef.current) galleryFileRef.current.value=''; return; }
    ...
    try {
      ...
      const d = await r.json();
      if (d.success) {
        const r2 = await fetch('/api/homepage-gallery', { method:'POST', headers: jsonH(), body: JSON.stringify({ image_url: d.url, caption: galleryCaption }) });
        const d2 = await r2.json();
        if (d2.success) { showToast('Image added to gallery!'); setGalleryCaption(''); fetchGallery(); }
        else showToast(d2.message || 'Failed to save', 'error');
      } else showToast(d.message || 'Upload failed', 'error');
    } catch { showToast('Upload failed', 'error'); }
    finally { setGalleryUploading(false); if(galleryFileRef.current) galleryFileRef.current.value=''; }
  };
  const handleDeleteGallery = async id => { if(!confirm('Remove this image from gallery?'))return; setGalleryLoading(true); try{const r=await fetch(`/api/homepage-gallery?id=${id}`,{method:'DELETE',headers:authH()});const d=await r.json();if(d.success){showToast('Removed!');fetchGallery();}else showToast(d.message||'Failed','error');}catch{showToast('Failed','error');}finally{setGalleryLoading(false);} };
  ...
  return (
    <div style={{ background: 'linear-gradient(160deg,#fdf6f0 0%,#fef0f5 40%,#f5f0fe 100%)', minHeight: '100vh', fontFamily: "'Jost',sans-serif" }}>
        ...
        {activeTab==='events-news' && (
          <div>
            ...
            <div style={{display:'flex',gap:8,marginBottom:24}}>
              {[{id:'events',label:'Events',icon:Calendar},{id:'news',label:'News',icon:MessageCircle},{id:'gallery',label:'Gallery',icon:Camera}].map(st=>{const I=st.icon;return(<button key={st.id} onClick={()=>setEnSubTab(st.id)} className={`flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-medium transition-all ${enSubTab===st.id?'adm-tab-active':'adm-tab-inactive'}`} style={{fontFamily:"'Jost',sans-serif"}}><I className="h-4 w-4"/>{st.label}</button>);})}
            </div>
            {enSubTab==='events'&&(
              <div>
            ...
            {enSubTab==='gallery'&&(
              <div>
                <div style={{marginBottom:24}}>
                  <h3 style={{fontFamily:"'Cormorant Garamond',Georgia,serif",fontWeight:400,color:'#3d2535',fontSize:'1.25rem',marginBottom:6}}>Homepage Gallery</h3>
                  <p style={{color:'#9a7888',fontFamily:"'Jost',sans-serif",fontWeight:300,fontSize:'0.85rem'}}>Images uploaded here appear as a slideshow on the home page. Only JPEG format is accepted.</p>
                </div>

                {/* Upload area */}
                <div className="adm-glass-card rounded-2xl p-6 mb-8" style={{border:'1px solid rgba(215,170,185,0.3)'}}>
                  <h4 style={{fontFamily:"'Jost',sans-serif",fontWeight:500,color:'#5a3548',fontSize:'0.9rem',marginBottom:14,textTransform:'uppercase',letterSpacing:'0.07em'}}>Upload New Image</h4>
                  <div style={{marginBottom:12}}>
                    <label style={{color:'#7a6070',fontFamily:"'Jost',sans-serif",fontWeight:400,fontSize:'0.85rem',display:'block',marginBottom:7}}>Caption <span style={{color:'#c4a0b0',fontWeight:300}}>(optional)</span></label>
                    <input type="text" value={galleryCaption} onChange={e=>setGalleryCaption(e.target.value)} placeholder="e.g., Annual CME Workshop 2025" className="adm-input w-full px-4 py-3" style={{maxWidth:480}}/>
                  </div>
                  <input type="file" ref={galleryFileRef} accept=".jpg,.jpeg" style={{display:'none'}} onChange={handleGalleryFileSelect}/>
                  <div
                    className={`adm-upload-area ${galleryUploading?'':'cursor-pointer'}`}
                    onClick={()=>!galleryUploading&&galleryFileRef.current?.click()}
                    style={{maxWidth:480}}
                  >
                    {galleryUploading?(
                      <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:10}}>
                        <Loader2 style={{width:26,height:26,color:'#c9608c',animation:'spin 1s linear infinite'}}/>
                        <p style={{color:'#9a7888',fontFamily:"'Jost',sans-serif",fontWeight:300,fontSize:'0.9rem'}}>Uploading…</p>
                      </div>
                    ):(
                      <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:10}}>
                        <Camera style={{width:28,height:28,color:'#c4a0b0'}}/>
                        <p style={{color:'#9a7888',fontFamily:"'Jost',sans-serif",fontWeight:400,fontSize:'0.92rem'}}>Click to select a JPEG image</p>
                        <p style={{color:'#c4a0b0',fontFamily:"'Jost',sans-serif",fontWeight:300,fontSize:'0.78rem'}}>Only .jpg / .jpeg files accepted</p>
                      </div>
                    )}
                  </div>
                </div>
                ...
                {galleryLoading&&!gallery.length?(
                  <div style={{textAlign:'center',padding:40}}><Loader2 className="h-8 w-8 animate-spin mx-auto" style={{color:'#c9608c'}}/></div>
                ):gallery.length===0?(
                  <div style={{textAlign:'center',padding:'40px 0'}}>
                    <Camera className="h-12 w-12 mx-auto mb-4" style={{color:'#e0c8d0'}}/>
                    <p style={{color:'#9a7888',fontFamily:"'Jost',sans-serif",fontWeight:300}}>No images uploaded yet</p>
                  </div>
                ):(
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                    {gallery.map(img => {
                      const cx = img.crop_x ?? 0, cy = img.crop_y ?? 0, cw = img.crop_w ?? 100, ch = img.crop_h ?? 100;
                      const isFullCrop = cw >= 99.9 && ch >= 99.9;
                      const bsW = isFullCrop ? 'cover' : `${(10000 / cw).toFixed(1)}%`;
                      const bsH = isFullCrop ? null  : `${(10000 / ch).toFixed(1)}%`;
                      const bpX = (!isFullCrop && (100 - cw) > 0.1) ? Math.min(100, (cx * 100) / (100 - cw)).toFixed(1) : 0;
                      const bpY = (!isFullCrop && (100 - ch) > 0.1) ? Math.min(100, (cy * 100) / (100 - ch)).toFixed(1) : 0;
                      const thumbBg = isFullCrop
                        ? { backgroundSize: 'cover', backgroundPosition: 'center center' }
                        : { backgroundSize: `${bsW} ${bsH}`, backgroundPosition: `${bpX}% ${bpY}%` };
                      return (
                        <div key={img.id} className="adm-glass-card rounded-2xl overflow-hidden" style={{border:'1px solid rgba(215,170,185,0.25)'}}>
                          {/* Cropped preview */}
                          <div style={{position:'relative',aspectRatio:'4/3',background:'linear-gradient(135deg,#fdf0f5,#f5f0fe)',overflow:'hidden'}}>
                            <div style={{position:'absolute',inset:0,backgroundImage:`url(${img.image_url})`,backgroundRepeat:'no-repeat',...thumbBg}}/>
                          </div>
                          {/* Info + buttons */}
                          <div style={{padding:'10px 12px 12px',background:'rgba(255,248,252,0.7)'}}>
                            <p style={{color:'#7a6070',fontFamily:"'Jost',sans-serif",fontWeight:300,fontSize:'0.8rem',marginBottom:10,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>
                              {img.caption || <span style={{color:'#c4a0b0',fontStyle:'italic'}}>No caption</span>}
                            </p>
                            <div style={{display:'flex',gap:7,justifyContent:'flex-end'}}>
                              <button onClick={() => openCropModal(img)}
                                style={{display:'flex',alignItems:'center',gap:5,padding:'6px 14px',borderRadius:9,background:'linear-gradient(135deg,#e8a0b0,#c9608c)',color:'white',border:'none',fontFamily:"'Jost',sans-serif",fontSize:'0.77rem',fontWeight:500,cursor:'pointer',transition:'opacity 0.2s'}}>
                                <Edit2 className="h-3 w-3"/>Adjust Crop
                              </button>
                              <button onClick={() => handleDeleteGallery(img.id)} className="adm-delete-btn p-1.5 rounded-lg" title="Remove image"><Trash2 className="h-4 w-4"/></button>
                            </div>
                          </div>
                        </div>
                      );
      ...
      {/* ─── Crop Modal ──────────────────────────────────────────────────── */}
      {showCropModal && cropModalImage && (
        <div style={{position:'fixed',inset:0,zIndex:9999,background:'rgba(20,8,16,0.82)',display:'flex',alignItems:'center',justifyContent:'center',padding:'20px'}}
          onClick={e => { if (e.target === e.currentTarget) setShowCropModal(false); }}>
          <div style={{background:'rgba(255,248,252,0.97)',borderRadius:20,width:'100%',maxWidth:780,boxShadow:'0 32px 80px rgba(0,0,0,0.45)',overflow:'hidden',display:'flex',flexDirection:'column',maxHeight:'90vh'}}>

            {/* Header */}
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'16px 20px',borderBottom:'1px solid rgba(215,170,185,0.25)'}}>
              <h3 style={{fontFamily:"'Cormorant Garamond',Georgia,serif",fontWeight:500,color:'#3d2535',fontSize:'1.3rem',margin:0}}>Adjust Crop</h3>
              <button onClick={() => setShowCropModal(false)} style={{background:'none',border:'none',cursor:'pointer',color:'#9a7888',padding:4,display:'flex',alignItems:'center',justifyContent:'center',borderRadius:8}}><X className="h-5 w-5"/></button>
            </div>

            {/* Image + crop box */}
            <div style={{flex:1,overflow:'auto',padding:20}}>
              <p style={{color:'#9a7888',fontFamily:"'Jost',sans-serif",fontWeight:300,fontSize:'0.83rem',marginBottom:12,textAlign:'center'}}>Drag the box to move it. Pull its edges or corners to resize. Whatever is inside the box will appear on the home page.</p>

              {/* Container */}
              <div ref={cropContainerRef} style={{position:'relative',width:'100%',userSelect:'none',cursor:'crosshair',background:'#1a0d14',borderRadius:12,overflow:'hidden',lineHeight:0}}>
                <img src={cropModalImage.image_url} alt="crop" style={{width:'100%',height:'auto',display:'block',opacity:0.85,pointerEvents:'none'}}/>

                {/* Dark overlay — 4 quadrants around the crop box */}
                {/* top bar */}
                <div style={{position:'absolute',left:0,right:0,top:0,height:`${cropModalBox.y}%`,background:'rgba(10,4,8,0.62)',pointerEvents:'none'}}/>
                {/* bottom bar */}
                <div style={{position:'absolute',left:0,right:0,bottom:0,height:`${100 - cropModalBox.y - cropModalBox.h}%`,background:'rgba(10,4,8,0.62)',pointerEvents:'none'}}/>
                {/* left bar */}
                <div style={{position:'absolute',top:`${cropModalBox.y}%`,height:`${cropModalBox.h}%`,left:0,width:`${cropModalBox.x}%`,background:'rgba(10,4,8,0.62)',pointerEvents:'none'}}/>
                {/* right bar */}
                <div style={{position:'absolute',top:`${cropModalBox.y}%`,height:`${cropModalBox.h}%`,right:0,width:`${100 - cropModalBox.x - cropModalBox.w}%`,background:'rgba(10,4,8,0.62)',pointerEvents:'none'}}/>

                {/* Crop box */}
                <div style={{
                  position:'absolute',
                  left:`${cropModalBox.x}%`, top:`${cropModalBox.y}%`,
                  width:`${cropModalBox.w}%`, height:`${cropModalBox.h}%`,
                  boxSizing:'border-box',
                  border:'2px solid rgba(255,255,255,0.92)',
                  boxShadow:'0 0 0 1px rgba(201,96,140,0.5)',
                  cursor:'move',
                }}>
                  {/* Move handle (inner area) */}
                  <div style={{position:'absolute',inset:0,cursor:'move'}} onMouseDown={e => startCropDrag(e, 'move')}/>

                  {/* Rule-of-thirds grid */}
                  <div style={{position:'absolute',inset:0,pointerEvents:'none'}}>
                    <div style={{position:'absolute',left:'33.33%',top:0,bottom:0,width:'1px',background:'rgba(255,255,255,0.22)'}}/>
                    <div style={{position:'absolute',left:'66.66%',top:0,bottom:0,width:'1px',background:'rgba(255,255,255,0.22)'}}/>
                    <div style={{position:'absolute',top:'33.33%',left:0,right:0,height:'1px',background:'rgba(255,255,255,0.22)'}}/>
                    <div style={{position:'absolute',top:'66.66%',left:0,right:0,height:'1px',background:'rgba(255,255,255,0.22)'}}/>
                  </div>
                  ...
                  {[
                    ...
                    <div key={h.pos} onMouseDown={e => startCropDrag(e, h.pos)}
                      style={{position:'absolute',width:12,height:12,background:'white',border:'2px solid #c9608c',borderRadius:2,boxShadow:'0 2px 6px rgba(0,0,0,0.4)',...h.style}}/>
                  ))}

                  {/* Edge handles */}
                  {[
                    { pos: 'n', style: { top: -4, left: '50%', transform: 'translateX(-50%)', cursor: 'n-resize', width: 24, height: 8 } },
                    { pos: 's', style: { bottom: -4, left: '50%', transform: 'translateX(-50%)', cursor: 's-resize', width: 24, height: 8 } },
                    { pos: 'w', style: { left: -4, top: '50%', transform: 'translateY(-50%)', cursor: 'w-resize', width: 8, height: 24 } },
                    { pos: 'e', style: { right: -4, top: '50%', transform: 'translateY(-50%)', cursor: 'e-resize', width: 8, height: 24 } },
                  ].map(h => (
                    <div key={h.pos} onMouseDown={e => startCropDrag(e, h.pos)}
                      style={{position:'absolute',background:'rgba(255,255,255,0.9)',border:'2px solid #c9608c',borderRadius:3,boxShadow:'0 2px 5px rgba(0,0,0,0.35)',...h.style}}/>
                  ))}
                </div>
              </div>

              {/* Crop info */}
              <p style={{color:'#b09098',fontFamily:"'Jost',sans-serif",fontWeight:300,fontSize:'0.76rem',marginTop:10,textAlign:'center'}}>
                Crop: x {cropModalBox.x.toFixed(1)}% &nbsp; y {cropModalBox.y.toFixed(1)}% &nbsp; w {cropModalBox.w.toFixed(1)}% &nbsp; h {cropModalBox.h.toFixed(1)}%
              </p>
            </div>

            {/* Footer buttons */}
            <div style={{display:'flex',gap:10,padding:'14px 20px',borderTop:'1px solid rgba(215,170,185,0.2)',justifyContent:'space-between',flexWrap:'wrap'}}>
              <button onClick={() => setCropModalBox({ x: 0, y: 0, w: 100, h: 100 })}
                style={{padding:'8px 18px',borderRadius:10,border:'1px solid rgba(200,150,165,0.35)',background:'rgba(255,248,252,0.8)',color:'#9a7888',fontFamily:"'Jost',sans-serif",fontSize:'0.82rem',cursor:'pointer',transition:'all 0.2s'}}>
                Reset to Full
              </button>
              <div style={{display:'flex',gap:10}}>
                <button onClick={() => setShowCropModal(false)}
                  style={{padding:'8px 18px',borderRadius:10,border:'1px solid rgba(200,150,165,0.35)',background:'rgba(255,248,252,0.8)',color:'#9a7888',fontFamily:"'Jost',sans-serif",fontSize:'0.82rem',cursor:'pointer'}}>
                  Cancel
                </button>
                <button onClick={handleApplyCrop} disabled={cropSaving}
                  style={{display:'flex',alignItems:'center',gap:6,padding:'8px 22px',borderRadius:10,background:'linear-gradient(135deg,#e8a0b0,#c9608c)',color:'white',border:'none',fontFamily:"'Jost',sans-serif",fontSize:'0.85rem',fontWeight:500,cursor:'pointer',opacity:cropSaving?0.7:1,transition:'opacity 0.2s'}}>
                  {cropSaving ? <Loader2 className="h-4 w-4 animate-spin"/> : <Save className="h-4 w-4"/>}
                  Apply Crop
                </button>
              </div>
            </div>
