            ...
            {enSubTab==='gallery'&&(
              <div>
                ...
                {galleryLoading&&!gallery.length?(
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                    {gallery.map(img=>(
                      <div key={img.id} className="adm-glass-card rounded-2xl overflow-hidden group" style={{border:'1px solid rgba(215,170,185,0.25)'}}>
                        <div style={{position:'relative',aspectRatio:'16/9',background:'linear-gradient(135deg,#fdf0f5,#f5f0fe)',overflow:'hidden'}}>
                          <img src={img.image_url} alt={img.caption||'Gallery'} style={{width:'100%',height:'100%',objectFit:'cover',display:'block',transition:'transform 0.35s ease'}} onMouseEnter={e=>e.currentTarget.style.transform='scale(1.05)'} onMouseLeave={e=>e.currentTarget.style.transform='scale(1)'}/>
                        </div>
                        <div style={{padding:'10px 14px',display:'flex',alignItems:'center',justifyContent:'space-between',gap:8}}>
                          <p style={{color:'#7a6070',fontFamily:"'Jost',sans-serif",fontWeight:300,fontSize:'0.82rem',flex:1,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{img.caption||<span style={{color:'#c4a0b0',fontStyle:'italic'}}>No caption</span>}</p>
                          <button onClick={()=>handleDeleteGallery(img.id)} className="adm-delete-btn p-1.5 rounded-lg flex-shrink-0" title="Remove image"><Trash2 className="h-4 w-4"/></button>
                        </div>
                      </div>
                    ))}
                  </div>
